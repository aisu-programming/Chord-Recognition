...
def build_model():
    model = Sequential()
    # model.add(Dense(24, input_shape=(frames_per_data, 24), activation='relu'))
    # model.add(Flatten())
    model.add(Dense(480, input_shape=(360, ), activation='relu'))
...
def readAndSplitData(train_file_index):
    ...
    Y['train'] = np.asarray(Y['train']).astype(np.int16)
    X['validation'] = np.asarray(X['validation']).astype(np.float32)
    Y['validation'] = np.asarray(Y['validation']).astype(np.int16)
...
def estimate_and_write_to_file(model_dict):
    ...
    for song_index in range(file_amount):
        ...
        for model_name, model in model_dict.items():
            ...
            Y_pred = Y_pred[:, one_side_frames].reshape(len(Y_pred), )
...
def check_all_same(labels):
    label_check = labels[0]
    for label in labels:
        if label_check != label: return False
    return True
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(BATCH_SIZE):
            ...
            if check_all_same(Y['train'][ti-one_side_frames:ti+one_side_frames+1]):
                X_train_batch.append(X['train'][ti-one_side_frames:ti+one_side_frames+1])
                Y_train_batch.append(to_categorical(Y['train'][ti], num_classes=len(mapping_dictionary)))
            ...
            if check_all_same(Y['validation'][ti-one_side_frames:ti+one_side_frames+1]):
                X_validation_batch.append(X['validation'][vi-one_side_frames:vi+one_side_frames+1])
                Y_validation_batch.append(to_categorical(Y['validation'][ti], num_classes=len(mapping_dictionary)))


        print(f'epoch: {epoch+1}/{EPOCHS} | batch_size: {BATCH_SIZE}')
        history = model.fit(
            np.array(X_train_batch).reshape(len(X_train_batch), frames_per_data*24),
            np.array(Y_train_batch), # .reshape(len(Y_train_batch), len(mapping_dictionary)),
            validation_data=(
                np.array(X_validation_batch).reshape(len(X_validation_batch), frames_per_data*24),
                np.array(Y_validation_batch)  # .reshape(len(Y_validation_batch), len(mapping_dictionary))
            ),
    ...
    ''' Save figure & model '''
    if data_divide_amount == 1: rename_save_directory = f'{rename_save_directory}-MFS'
    else: rename_save_directory = f'{rename_save_directory}-DMFS'
