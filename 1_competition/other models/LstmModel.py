...
def build_model():
    model = Sequential()
    model.add(LSTM(250, input_length=frames_before, input_dim=24, return_sequences=False))
...
def estimate_and_write_to_file(model_dict):
    ...
    for song_index in range(file_amount):
        ...
        for i in range(original_data_length):
            X.append(data[i:i+frames_before][:24])
    ...
    if load_exist_model:
        ...
        for epoch in range(EPOCHS):
            ...
            for i in range(BATCH_SIZE):
                ...
                Y_train_batch.append(Y['train'][ti+frames_before-1])
                X_validation_batch.append(X['validation'][vi:vi+frames_before])
                Y_validation_batch.append(Y['validation'][vi+frames_before-1])
