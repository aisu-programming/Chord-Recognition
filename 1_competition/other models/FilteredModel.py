...
def build_model():
    ...
    model.add(Flatten())
    for _ in range(15):
        model.add(Dense(24*frames_per_data, activation='relu'))
        model.add(BatchNormalization())
...
def readAndSplitData(train_file_index):
    ...
    for song_index in range(file_amount):
        ...
        data = gaussian_filter(data.values, SIGMA)
...
def estimate_and_write_to_file(model_dict):
    ...
    for song_index in range(file_amount):
        ...
        # original_data = gaussian_filter(original_data.values, SIGMA)
...
def main():
    ...
    ''' Save figure & model '''
    if data_divide_amount == 1: rename_save_directory = f'{rename_save_directory}-Fil'
    else: rename_save_directory = f'{rename_save_directory}-Div-Fil'
