...
from tensorflow.keras.layers import (
    ZeroPadding2D, Conv2D, MaxPooling2D, Dropout, Reshape, Permute, Bidirectional, LSTM, BatchNormalization, Dense
)
...
def build_model():
    ...
    model.add(ZeroPadding2D(padding=1, input_shape=(frames_per_data, 12*2, 1)))
    ...
    model.add(Bidirectional(LSTM(128, input_length=5, input_dim=64)))
    model.add(BatchNormalization())
    model.add(Dense(len(mapping_dictionary), activation='softmax'))
    model.compile(loss="categorical_crossentropy", optimizer="adam", metrics=["accuracy"])
    # plot_model(model, to_file=f'{save_directory}/plot_model.png', show_shapes=True)
    print('')
    model.summary()
    print('')
    return model
...
def readAndSplitData(train_file_index):
    ...
    for song_index in range(file_amount):
        ...
        for _ in range(frames_per_data-1):
            if song_index in train_file_index:
                X['train'].append(filling_row*2)
                Y['train'].append(0)
            else:
                X['validation'].append(filling_row*2)
        ...
        for row in data:
            ...
            if song_index in train_file_index:
                X['train'].append(row[:12]*2)
                Y['train'].append(label)
            else:
                X['validation'].append(row[:12]*2)
...
def estimate_and_write_to_file(model_dict):
    ...
    for song_index in range(file_amount):
        ...
        filling_row = [0.0] * 12
        data = []
        for _ in range(frames_per_data-1): data.append(filling_row*2)
        for row in original_data: data.append(row[:12]*2)

        X = []
        for i in range(original_data_length):
            X.append(data[i:i+frames_per_data])
            
        X = np.array(X).astype(np.float32) # .reshape(original_data_length, frames_per_data, 12*2)
...
def main():
    ...
    train_index_array = np.arange(0, len(X['train'])-frames_per_data+1)
    validation_index_array = np.arange(0, len(X['validation'])-frames_per_data+1)
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(BATCH_SIZE):
            ...
            X_train_batch.append(X['train'][ti:ti+frames_per_data])
            Y_train_batch.append(Y['train'][ti+frames_per_data-1])
            X_validation_batch.append(X['validation'][vi:vi+frames_per_data])
            Y_validation_batch.append(Y['validation'][vi+frames_per_data-1])

        print(np.array(X_train_batch).shape)

        print(f'epoch: {epoch+1}/{EPOCHS} | batch_size: {BATCH_SIZE}')
        history = model.fit(
            np.array(X_train_batch).reshape(BATCH_SIZE, frames_per_data, 12*2, 1),
            np.array(Y_train_batch), # .reshape(BATCH_SIZE, len(mapping_dictionary)),
            validation_data=(
                np.array(X_validation_batch).reshape(BATCH_SIZE, frames_per_data, 12*2, 1),
