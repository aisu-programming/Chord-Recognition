...
def build_model():
    model = Sequential()
    model.add(Dense(28, input_shape=(frames_per_data, 24), activation='relu'))
...
def estimate_and_write_to_file(model_dict):
    ...
    for song_index in range(file_amount):
        ...
        for model_name, model in model_dict.items():
            ...
            Y_class = []
            for frame_index, frame in enumerate(Y_predicts):
                Y_class.append({})
                for i in range(-1 * min(frame_index, one_side_frames), min(len(Y_predicts)-frame_index-1, one_side_frames)+1):
                    if frame[i] not in Y_class[frame_index]: Y_class[frame_index][frame[i]] = 1
                    else: Y_class[frame_index][frame[i]] += 1
                Y_class[frame_index] = [ k for k, v in sorted(Y_class[frame_index].items(), key=lambda y: y[1], reverse=True)][0]

            with open(f'{data_directory}/{song_index+1}/est_file_{model_name}.txt', mode='w') as f:
                index_now = 0
                index_last = 0
                while index_now < len(Y_class):
                    if (index_now == len(Y_class) - 1) or (Y_class[index_now] != Y_class[index_now+1]):
                        for k, v in mapping_dictionary.items():
                            if v == Y_class[index_now]:
...
def main():
    ...
    ''' Save figure & model '''
    if data_divide_amount == 1: rename_save_directory = f'{rename_save_directory}-m2m'
    else: rename_save_directory = f'{rename_save_directory}-m2m'
